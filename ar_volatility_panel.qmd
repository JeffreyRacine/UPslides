...
- After this, R functions will act appropriately (e.g., `plot()` recognizes `ts()` so the horizontal axis is time, lines connect points, etc.)
  ...
  print(x)
  plot(x)
  ## Excample 2 - create simulated data from an ARIMA model that begins on the 2nd
  ## Quarter of 1959
...
ghat.4 <- npreg(y~lag(y,-1)+lag(y,-2)+lag(y,-3)+lag(y,-4))
## Re-fit the model using only 1 lag
ghat.1 <- npreg(y~lag(y,-1))
## cbind(ts(fitted(ghat.4),start=5),ts(fitted(ghat.1),start=2))
## Plot the data and nonparametric fit
plot(y,lwd=2)
lines(ts(fitted(ghat.4),start=5),col=2,lty=2,lwd=2)
points(ts(fitted(ghat.1),start=2),col=3)
...
## Nonparametric AR Models (Lag Inference) {.smaller}
...
- The generalized local polynomial estimator [@HALL_RACINE:2015] uses cross-validation to select the polynomial degree vector and bandwidth vector
...
- We will also fit a correctly specified parametric AR(1) model and compare the first few fitted values
  ...
  model <- npglpreg(y~y.lag.1+y.lag.2+y.lag.3+y.lag.4,nmulti=5,degree.max=5)
  ```

## Nonparametric AR Models {.smaller}
...
## linear AR(1) models
cbind(fitted(model),fitted(model.lm))[1:10,]
```

## Nonparametric AR Models
- This method correctly selects the degree for the first lag (the AR(1) process is a linear process, so the degree `r model$degree[1]` with a large bandwidth is appropriate)

- It correctly selects a large bandwidth and a constant function (degree `r model$degree[2]`) for the remaining lags resulting in a globally linear model that is a function of the first lag of $Y$ only
