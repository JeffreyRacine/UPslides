...
- Skipping technical derivations, in the multivariate case we can show that
    \begin{equation*}
      \operatorname{bias}\hat f(x) \approx \sum_{s=1}^q \frac{h_s^2}{2}f_{ss}(x)\kappa_2,
    \end{equation*}
     based on a *second order kernel* where $0< \kappa_2 = \int z^2 K(z)\, dz<\infty$, where $f_{ss}(x)$ is the second-order derivative of $f(x)$ with respect to $x_s$
     
- We can also show in the multivariate case that 
    \begin{equation*}
...
- We continue to use likelihood and least-squares data-driven methods (typically cross-validation) for bandwidth selection when $q\ge1$ (optimal bandwidths differ across variables, i.e.,  $h_1\ne h_2$, etc.)
